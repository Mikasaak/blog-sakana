...
const { data: categoriesData } = await useAsyncData('websites', async () => {
  const result = await queryCollection('websites').all()
  // Nuxt Content v3 treats JSON file as a document.
  // result[0] is the document for 'websites.json'.
  // result[0].meta.body contains the JSON array.
  // We need to cast it to our type.
  const doc = result?.[0]
